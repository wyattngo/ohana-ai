...
- **`ruff` LUÔN chạy với `--no-cache`, và version bị PIN.** `.ruff_cache` do một bản ruff cũ ghi ra **không bị vô hiệu khi ruff nâng cấp**: `ruff check .` trả `All checks passed!` trong khi `ruff check . --no-cache` trả 4 lỗi trên **cùng source**. Gate local nói dối, và `GATE_FULL` của ADP nuốt lời nói dối đó vào EVIDENCE — spec 08 E0 đã bị stamp một bước ruff xanh giả trước khi phát hiện. Rà lại 22 phase DONE dưới ruff pin: **19/22 không tái lập được** `ruff check` (ISSUE-019). Vì vậy `pyproject.toml` pin **cả 4 dev tool** — `ruff==0.15.22` · `mypy==2.3.0` · `pytest==9.1.1` · `pytest-asyncio==1.4.0` (KHÔNG `>=`) — gate phải là hàm của source, không phải của ngày cài đặt.
  ...
  ⚠️ **Runtime deps vẫn `>=`** (`openai>=1.30` thực cài 2.45 — SDK mà cả `TogetherClient` lẫn `TogetherEmbedder` dùng). Cùng lớp rủi ro, chưa ai nhận (ISSUE-019 action 6).

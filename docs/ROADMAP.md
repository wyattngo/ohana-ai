...
| 4 | Size theo dáng — "1m6 50kg mặc gì" | Size-chart logic + body input | size chart + rule | Sai size → đổi/trả | `GD0-WIKI` |
| 5 | Phí ship — "ship Q7 nhiêu" | Parametric | carrier API / bảng phí | Sai phí | `GD0-TOOLS` / `GD1-SHIP` |
| 6 | Thời gian giao — "bao lâu", "kịp T7?" | Chung: RAG · Cụ thể: tracking | Wiki / `order_status` | Hứa sai deadline | `GD0-WIKI` / `GD1-SHIP` |
| 7 | Chính sách — "COD đc ko", "freeship?" | Policy RAG | Wiki | Thấp (grounded) | `GD0-WIKI` |
| 8 | Uy tín/sợ scam — "shop real ko" | Policy RAG + tone trấn an | Wiki | Thấp, nhạy tone | `GD0-WIKI` |
...
| `GD0-SHOPS` | `shops` table + JWT từ real onboard (không stub) | Onboard tạo shop thật → JWT mang `shop_id` thật | internal | — |
...
| `GD0-WIKI` | Real wiki corpus ingest (batch + delta) + admin upload | Corpus thật ingest; `search_wiki` trả chunk đúng chủ đề | **external** | Tân (PRE-003) |
| `GD0-TOOLS` | F2 tools `shipping_info` / `product_info` / `account_lookup` trên endpoint thật | 3 tool gọi platform API thật, param validation trước execute | **external** | Tân (PRE-002) |
...
GD0-EMBED          ── không chờ ai; mở khoá GD0-WIKI dùng được cho khách thật
GD0-EVAL           ── không chờ ai; là PREREQUISITE của GD0-INTENT
GD0-SHOPS / METER / ROUTER / OBS / RESIDENCY  ── không chờ ai

⛔ GD0-ZALO / WIKI / TOOLS / WINDOW  ── kẹt PRE-002/003/004 ở Tân
      └─ [Gate GĐ0 acceptance]

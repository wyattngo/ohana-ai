...
db/           models.py (tenant-first) · repos (Conversation/PendingReply/Message/ShopProfile) · session · migrations/ (Alembic 0001–0007)
...
- **Hai bộ đếm, hai câu hỏi:** `adp-status.sh` = *phase đã ký* (32/42) · `adp-roadmap.sh` = *work item thật* (internal 11/28). Mục tiêu 100% = mẫu số `internal`, không gộp `external` chờ bên thứ ba. Số roadmap thấp hơn vì mẫu số ĐÚNG hơn — không phải tiến độ xấu đi (DEC-OHANA-03).
...
- ⏳ Spec 03 (acceptance backfill, 0/10, 4 BLOCKED chờ Tân) — **Phase 1 giờ là `CANCELLED`**, superseded bởi spec 11 (blocker PRE-007 ACCEPTED 2026-07-19 mà không ai gỡ; L1 `GD0-SHOPS` từng map vào HAI chủ, nay chỉ còn spec 11). Migration đã dịch **bốn lần** sang `0008`/`0009`/`0010` (spec 08 lấy `0004`, spec 09 `0005`, spec 10 `0006`, spec 11 `0007`). Số cấp theo thứ tự LAND, không theo thứ tự lập kế hoạch. ⚠️ §8 của spec 03 từng lệch với §Files của chính nó (hai lớp số khác nhau) — đồng bộ 2026-07-20; xem ISSUE-021.
- **Gate hiện tại:** pytest xanh (`-m 'not live'`, **185 test**, 6 live deselected) · ruff sạch (`--no-cache`) · mypy 0 lỗi / **53 file** — scope = toàn bộ code sản phẩm, không loại thư mục nào (2026-07-21).
- **Live (chạy tay, ngoài CI):** `test_together_live.py` 2 · `test_wiki_rag_live.py` 4 — cả 6 PASS 2026-07-19 với key thật.
- ✅ `main` đồng bộ `origin/main` (`github.com:wyattngo/ohana-ai`) — spec 11 S2+S3 đã push. STATE_HASH `1f9a1ba36193` @ spec 11 S3. **4 commit spec 11 giờ ĐÃ qua CI và XANH**: tip `16bc429` chạy đủ 11 step trên tree chứa cả S2+S3 (2026-07-21). Nghi ngờ hình-dạng-ISSUE-019 với spec 11 đã đóng — lần này biết trước, và CI xác nhận đúng như dự đoán. ⚠️ Lưu ý per-commit: CI không chạy riêng từng commit S2/S3; bằng chứng là tree tại tip xanh, không phải mỗi commit trung gian. `16bc429` đứng SAU 3 commit chore/ci ngoài-ADP (AI-coder profile + map gate), không phải phase ADP.
- **OPEN:** ISSUE-010 (alerting — `app/alert_service.py` chưa port, 429 không được đếm ở đâu) · ISSUE-021 (L1 gỡ `shipping_info` nhưng spec 03 Phase 4 frozen vẫn khai 3 read-tool) · ISSUE-022 (cap persona 2000 chưa đo) · ISSUE-023 (cap history 20/4000 chưa đo tokenizer).

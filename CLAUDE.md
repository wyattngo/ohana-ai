...
agent/        orchestrator (history load + cap kép) · llm_client · embedder · policy_gate
...
db/           models.py (tenant-first) · repos (Conversation/PendingReply/Message) · session · migrations/ (Alembic 0001–0006)
...
- **Hai bộ đếm, hai câu hỏi:** `adp-status.sh` = *phase đã ký* (28/38) · `adp-roadmap.sh` = *work item thật* (internal 10/27). Mục tiêu 100% = mẫu số `internal`, không gộp `external` chờ bên thứ ba. Số roadmap thấp hơn vì mẫu số ĐÚNG hơn — không phải tiến độ xấu đi (DEC-OHANA-03).
...
- **Gate hiện tại:** pytest xanh (`-m 'not live'`, **154 test**, 6 live deselected) · ruff sạch (`--no-cache`) · mypy 0 lỗi / **49 file** — scope = toàn bộ code sản phẩm, không loại thư mục nào (2026-07-20).

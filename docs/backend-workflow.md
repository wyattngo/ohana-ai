...
Webhook endpoint
  ...
  │
  ├─ Idempotent write (outbox pattern — 1 transaction):
  │     BEGIN;
  │       INSERT INTO wiho_webhook_seen(channel, platform_msg_id, shop_id, raw_event, received_at)
  │         ON CONFLICT (channel, platform_msg_id) DO NOTHING;
  │       -- Nếu row đã tồn tại → RETURN 200 ngay, không ghi outbox
  │       INSERT INTO wiho_outbox(event_id, payload, status='pending');
  │     COMMIT;
  │
...
Trigger từ scheduler (nhịp 0.5)
  ...
  │
  ├─ PII filter tin khách + 6 lượt trước khi build prompt
  │     ⚠️ Bắt buộc, không optional. Xem §5 và §7.6.
  │
...
- **Dữ liệu xuyên biên giới.** Nội dung tin khách gửi tới LLM foreign nằm trong phạm vi Nghị định 13/2023 + luật 2026. Ràng buộc: (a) **PII filter trước khi gửi LLM** — không phải "trước khi ra sandbox", mà **trước mọi LLM call**, ngay cả trong dev; (b) DPIA riêng cho từng nhà cung cấp LLM; (c) log destination cho audit. Không quyết ở tầng dưới.

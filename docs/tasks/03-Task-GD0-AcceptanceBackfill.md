...
- Files: `db/models.py` (add `Shop`, `WebhookEventLog`), `db/migrations/0003_*.py`, `auth/identity.py`, `bridge/zalo_sender.py`, `api/webhook.py`, `api/admin.py`, `agent/scheduler.py` (new).
...
- Files: `db/models.py` (add `CreditLedger`), `db/migrations/0004_*.py`, `agent/metering.py` (new), `api/middleware.py` (new).
...
- `db/migrations/` — Phase 1 (0003), Phase 5 (0004).
...
APPROACH: Add `Shop` model + Alembic 0003; extend `auth/identity.py` load `shop_id` từ DB thay stub; onboard endpoint `POST /api/admin/shops` (admin auth); JWT issuance include `shop_id` claim; adversarial test cross-shop.
ALLOWED_FILES: db/models.py, db/migrations/0003_shops.py, auth/identity.py, api/admin.py, tests/test_shops_onboard.py, tests/conftest.py, docs/reviews/, docs/tasks/03-Task-GD0-AcceptanceBackfill.md
...
2. Add `Shop` model + Alembic 0003 (columns: id, name, zalo_oa_id UNIQUE, plan_tier, created_at, is_active).
...
ALLOWED_FILES: bridge/zalo_sender.py, api/webhook.py, api/middleware.py, db/models.py, db/migrations/0004_webhook_log.py, tests/test_zalo_sender.py, tests/test_webhook_idempotency.py, tests/conftest.py, docs/reviews/, docs/tasks/03-Task-GD0-AcceptanceBackfill.md
...
9. `WebhookEventLog` model + Alembic 0004.
...
APPROACH: Add `CreditLedger` model + Alembic 0005; `agent/metering.py` implement debit + balance check; `api/middleware.py` wrap chat endpoint với metering + rate-limit (Redis-backed nếu có, in-memory fallback); adversarial bypass test.
ALLOWED_FILES: db/models.py, db/migrations/0005_credit_ledger.py, agent/metering.py, api/middleware.py, api/inbox.py, tests/test_credit_metering.py, tests/test_metering_bypass.py, tests/conftest.py, docs/reviews/, docs/tasks/03-Task-GD0-AcceptanceBackfill.md
...
23. `test_metering_bypass.py` (RED, adversarial): call `/api/chat/draft` với body claim `shop_id=X` nhưng JWT shop=Y → dùng shop từ JWT (không body); call với missing JWT → 401.
24. `CreditLedger` model + Alembic 0005.
...
ALLOWED_FILES: agent/scheduler.py, db/models.py, db/migrations/0006_conversation_window.py, api/inbox.py, tests/test_reactive_window.py, tests/conftest.py, pyproject.toml, docs/reviews/, docs/tasks/03-Task-GD0-AcceptanceBackfill.md
...
59. Add `last_inbound_at` column cho conversations + Alembic 0006.
...
- **Alembic 0003 (Phase 1):** `shops` table (`id UUID, name TEXT, zalo_oa_id TEXT UNIQUE, plan_tier TEXT, is_active BOOL, created_at TIMESTAMPTZ`).
- **Alembic 0004 (Phase 2):** `webhook_event_log` table (`event_id TEXT PRIMARY KEY, shop_id UUID FK, payload JSONB, signature TEXT, received_at TIMESTAMPTZ, processed_at TIMESTAMPTZ, status TEXT`).
- **Alembic 0005 (Phase 5):** `credit_ledger` table (`id UUID, shop_id UUID FK, delta INT, reason TEXT, conversation_id UUID FK nullable, balance_after INT, created_at TIMESTAMPTZ`). Index `(shop_id, created_at DESC)`.
- **Alembic 0006 (Phase 10):** ALTER `conversations` ADD COLUMN `last_inbound_at TIMESTAMPTZ`, `window_status TEXT DEFAULT 'active'` (active/warning/expired).
...
Credit metering bypass test (adversarial):
  1. POST /api/chat/draft với body {"shop_id": "X"} nhưng JWT shop=Y → debit trên Y (JWT wins)
